...
class NocViewApplication extends StatefulWidget {
  ...
  _NocViewApplicationState createState() =>
      _NocViewApplicationState(userGroup, sessionToken, Ids);
}
...
class _NocViewApplicationState extends State<NocViewApplication> {
  ...
  void NocView() async {
    String url = 'http://13.234.208.246api/auth/NocViewApplication';
    Map data = {
      "app_id": Ids,
    };
    ...
    for (int i = 0; i < treelog.length; i++) {
      ...
      // n_list.add(i);
      species.add(
          responseJSON['data']['timber_log'][i]['species_of_tree'].toString());
