...
class DivisonDashBoard extends StatefulWidget {
  ...
  DivisonDashBoard(
      ...
      this.userRange});
  @override
  _DivisonDashBoardState createState() => _DivisonDashBoardState(userId,
      userName, userEmail, sessionToken, userGroup, dropdownValue, userRange);
}
...
//------------------End--Pie--Chart--------------
  ...
  String url = " http://13.234.208.246api/auth/new_transit_pass_pdf/90/";
...
//-----------Approved------------------------
  ...
  void PendingApp() async {
    ...
    //---------------clear---------------
    const String url =
        'http://13.234.208.246api/auth/DfoPendingListViewApplication';
    Map data = {
...
//-----------Approved-------------
  void ApprovedApp() async {
    ...
    //----------clear-----------
    const String url =
        'http://13.234.208.246api/auth/DfoApprovedListViewApplication';
    Map data = {
      "area_range": SelectedRange == "All Range"
          ? ""
          : SelectedRange == null
...
//--end-Approve-------------------
  ...
  void DeemedApp() async {
    ...
    sr2.clear();
    const String url = 'http://13.234.208.246api/auth/DeemedApprovedList';
    final response = await http.get(Uri.parse(url), headers: <String, String>{
      'Content-Type': 'application/json',
      'Authorization': "token $sessionToken"
    });
  ...
  void NocForm() async {
    ...
    sr3.clear();
    const String url = 'http://13.234.208.246api/auth/NocListApplication/';
    final response = await http.get(Uri.parse(url), headers: <String, String>{
      'Content-Type': 'application/json',
      'Authorization': "token $sessionToken"
    });
  ...
  String OfficerStatus(
    ...
    if (division == true) {
      if (AppStatus == 'R') {
        ...
        return "Recommended by Division Officer";
      }
    } else if (forest == true) {
      if (otherState == false) {
        if (AppStatus == 'R') {
      ...
      if (log_updated_by_user == true) {
        if (AppStatus == 'R') {
          ...
          return 'Wood logs details have been be entered for Stage 2. Deputy Range Officer Recommendation Pending for Stage 2';
        }
      } else {
        if (AppStatus == 'R') {
          return 'Rejected by Forest Range Officer(Trees Cutting Approval)';
        } else {
          return 'Recommended by Forest Range Officer for cutting of Trees. Wood logs details to be entered.';
        }
      }
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        TextButton.icon(
                          ...
                          onPressed: () async {
                            await launch(
                                " http://13.234.208.246api/auth/summary_report/");
                          },
                          label: Text("Download"),
                        ),
                      ],
